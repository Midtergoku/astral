# O que o Astral é

> Criado em 18/09/2026 a pedido dele: *"é como se fosse uma apresentação de tudo que o nosso
> site propõe (...) conforme a gente vai dando os checks no nosso roadmap, isso também vai sendo
> incluso. (...) Você vai separar por áreas."*
>
> **Como este documento funciona:** cada área tem duas partes — **o que já está no ar** e **o que
> vem**. Quando um item do `roadmap-rpg.md` ganha o ✅, ele **muda de parte dentro da sua própria
> área**, e não vai para o fim do documento. Assim a apresentação continua legível em vez de
> virar um diário em ordem de execução.
>
> 🔴 **Regra deste arquivo: nada de vitrine.** Só entra em "já está no ar" o que eu medi
> funcionando. Promessa fica em "o que vem", com essa palavra.

---

**Astral — transforme seu edital em um plano de aprovação em poucos minutos.**
Para quem presta concurso militar. R$ 19,90/mês. Hoje em beta fechado.

---

## 1. 🎲 O RPG — estudar vira progressão

A aposta central: um concurseiro estuda sozinho por meses sem nenhum sinal de que está
avançando. O Astral transforma o estudo real em progressão visível — **e só em estudo real**.

### Já está no ar

| | O quê |
|---|---|
| **Patentes que mudam conforme o seu concurso** | 11 níveis, de 0 a 35.000 XP. Quem sobe um edital dos bombeiros ganha as patentes **dos bombeiros**; de PM, as da PM. São **6 tabelas** — bombeiros, marinha, aeronáutica, exército, PM e padrão — escolhidas pelo nome do edital |
| **XP que vem do trabalho** | sessão de estudo marcada = peso da matéria × 5 · cronômetro = 2 XP por minuto |
| **8 conquistas** | acendem sozinhas ao cruzar o número: primeiro XP, 1 hora estudada, 3 e 7 dias de sequência, níveis 3 e 5, primeiro edital, 5.000 XP |
| **15 habilidades secretas** | Orador de Guerra, Calculista, Alquimista, Médico de Combate, Navegador, Operador Cyber, Estrategista, Guardião da Lei… Destrancam com **70% de domínio na matéria** — e são secretas **de propósito**: o valor está em descobrir sem esperar |
| **Elas enferrujam** | 7 dias sem estudar a matéria = enferrujada · 14 dias = **fora de serviço**. A habilidade acompanha o seu estado atual, não o seu recorde — e o cartão diz há quanto tempo e o que fazer: **uma sessão** traz ela de volta |
| **A tag na barra superior** | `NÍVEL · TAG` — o lugar mais nobre da tela mostra **quem você é**, não qual plano você paga |
| **A ficha** | Cinco atributos no lugar de um número só: **Disciplina** (aparecer, e aparecer de novo) · **Resistência** (aguentar sentado) · **Amplitude** (não abandonar matéria) · **Doutrina** (o quanto já domina) · **Precisão** (ainda esperando o banco de questões). Cada um **diz de onde veio** — *"12 dias de estudo nos últimos 30, sequência de 5"* |
| **E ela não dá para forjar** | Os atributos são **calculados pelo servidor** a partir das suas sessões. Não existe campo para preencher: o número sai da conta |
| **74 condecorações** | Bronze, Prata, Ouro — e a **Condecoração Máxima**, que só cai quando as outras 73 caem. **22 são secretas** e nem aparecem até você descobri-las |
| **33 divisas** | A tag que você veste, uma de cada vez. Cada uma com raridade: comum, incomum, rara, lendária |
| **"Falta pouco"** | A tela mostra o que está a um passo. Ver um 8 de 10 puxa mais que uma lista de coisas trancadas |
| **A medalha se anuncia na hora** | No instante em que cai, com confete e a cor do metal. Uma conquista secreta descoberta três semanas depois, numa tabela, é uma linha de tabela — o valor está no instante |
| **Missões de hoje** | Três por dia, e elas **não mudam se você recarregar**. *"Estude 25 minutos"*, *"duas matérias diferentes"*, *"uma sessão antes das 9h"* — e o quanto já falta aparece em cada uma |
| **Campanhas** | Quatro trilhas de quatro etapas que **nunca expiram**: Apresentação ao Quartel · Operação Constância · Frente Ampla · Marcha de Resistência. A próxima etapa só abre quando a anterior fecha |
| **A prova é o chefe** | Sua data vira o chefe da campanha, no alto do painel: quanto falta, **o quanto você está preparado** (0 a 100) e **qual matéria atacar agora** — a que mais custa ponto, com nome. Contagem regressiva sem direção é só pressão; com direção, é plano |

### O que vem

| | O quê |
|---|---|
| **Árvore de habilidades** | um ponto a cada patente, gasto em **Infantaria** (constância), **Artilharia** (volume) ou **Inteligência** (precisão). Os ramos mudam *como* você joga, nunca *o que* você aprende |
| **Missões** | as **gerais**, que ficam para sempre depois de cumpridas, e as **diárias**. Sempre cumpridas estudando — nunca por abrir o aplicativo |
| **Catálogo de tags** | muitas, com cor própria e **raridade**: comum, incomum, rara, lendária |
| **O chefe tem data** | a sua prova vira o chefe da campanha, com contagem regressiva. Cada simulado é um sub-chefe |
| **Diário de campanha** | *"Dia 34 — 2h20 em Matemática. Domínio 58% → 61%. Desbloqueado: Calculista."* |
| **Prestígio** | trocar de concurso ou passar não apaga nada: você vira **veterano**, com marca permanente |

---

## 2. 📄 O edital vira plano

### Já está no ar

| | O quê |
|---|---|
| **Leitura do edital** | você sobe o PDF e a IA extrai as matérias e o peso de cada uma |
| **Cronograma** | as matérias viram sessões distribuídas, com o peso definindo quanto tempo cada uma recebe |
| **Professores por matéria** | a IA busca onde estudar cada matéria, e o resultado **fica salvo para sempre** na sua conta — não se refaz sozinho, não gasta de novo |

> 🔴 **A verdade que este documento não esconde:** em toda a história do Astral, **ninguém nunca
> subiu um edital**. A função existe, está testada e nunca foi usada de verdade. Isso não é
> defeito do produto — é o item nº 1 do roadmap geral, e depende de US$ 5 de crédito.

---

## 3. 📚 Banco de questões de provas antigas

### Já está no ar

Nada ainda. **As questões geradas por IA estão desligadas desde 31/07 de propósito** — eram a
única ação de custo recorrente, e questão de prova de verdade é melhor que questão inventada.

### O que vem

| | O quê |
|---|---|
| **Acervo de provas militares antigas** | de domínio público. **Medido em 17/09:** de uma prova real da EEAR com 96 questões, **78 saem utilizáveis automaticamente (81%)**, em 0,36 segundo, a custo zero. O gabarito vem no mesmo PDF |
| **Filtros de verdade** | por **banca**, por **matéria** e por **assunto dentro da matéria** — funções, porcentagem, crase. É o filtro que faz um acervo grande servir para alguma coisa |
| **Simulado** | uma incursão com relatório no fim: desempenho por matéria, cruzado com o peso do seu edital |
| **Caderno de erros** | o que você errou volta até você acertar |

> 💰 **Grátis × Pro:** o grátis tem **10 questões por dia** e **um simulado completo**, para
> sentir o produto inteiro uma vez. O Pro tem acesso ilimitado, filtro pelas matérias do **seu**
> edital, as provas mais recentes e o caderno de erros.

---

## 4. ⏱️ As ferramentas do dia a dia

### Já está no ar

| | O quê |
|---|---|
| **Cronômetro por matéria** | livre ou pomodoro; cada sessão fica registrada com matéria, duração e horário |
| **Calendário** | seus eventos e prazos |
| **Cronograma da semana** | o que estudar em cada dia |
| **Progresso por matéria** | quanto você domina de cada uma |
| **Minha conta** | seu plano e seus limites, num lugar só |

---

## 5. 🔒 O que sustenta tudo — e que ninguém vê

Esta área não aparece na tela, e é a que faz o produto poder existir.

### Já está no ar

| | O quê | Provado como |
|---|---|---|
| **Sua conta é só sua** | ninguém alcança seu progresso, nem com login válido, nem para ler, alterar, apagar ou se promover | teste que cria dois usuários reais e ataca um com o outro |
| **Seu progresso mora na conta** | sair, entrar e trocar de aparelho não perde nada — nem num celular sem nenhum cache | 9 checagens de 9, incluindo o número aparecendo na tela |
| **Duas abas abertas não se atropelam** | antes, uma apagava o trabalho da outra | controle de concorrência no próprio banco |
| **Backup que volta** | o plano grátis do Supabase **não faz backup nenhum** — este faz, e já foi restaurado e conferido linha a linha | 114 linhas conferidas uma a uma |
| **Funciona no celular** | nas 19 páginas, em telas de 360, 375 e 390 pixels: nada vaza, o menu abre e fecha, os alvos são grandes o bastante para o dedo | teste automatizado em navegador real |
| **Vigia de hora em hora** | se o site cair, chega e-mail | rotina automática |
| **Suas fontes não dependem do Google** | o site não espera um servidor de fora para desenhar o texto | 6 arquivos hospedados por nós |

### O que vem

| | O quê |
|---|---|
| **O servidor passa a calcular o XP** | hoje o navegador informa o total — e dá para escrever qualquer número. Inofensivo enquanto o XP não vale nada; **obrigatório antes de o XP destrancar qualquer coisa** |

---

## 6. 💳 Pagamento

### O que vem

Mercado Pago (não Stripe — não exige CNPJ). **Bloqueador conhecido:** a validação de assinatura
do webhook, que precisa estar pronta antes de qualquer cobrança real.

---

> **Última atualização:** 18/09/2026.
> Este arquivo acompanha o `roadmap-rpg.md` — cada ✅ de lá aparece aqui, dentro da área dele.
